...
class bAbITask(Task):
    base = os.path.join(project_path.base, "bgavDNC_src", "task_implementations", "bAbI")
    output_symbol = "-"
    pad_symbol = "*"
    ...
    def test(self, ntm, batch_size,device):
        """task.test(ntm,fd=[x, y, sequence_lengths, mask],batch_size)
        ...
        for ind, (inp, output) in enumerate(zip(self.x_test_stories.items(), self.y_test_stories.items())):
            ...
            for index in range(num_stories):
                ...
                #x, fd[1]: y, fd[2]: lengths, fd[3]: m
                #print("x.shape:",x.shape,"length:",lengths)
                #if input.size()[1]!=lengths:raise ValueError("input shape error")
                
                input =torch.tensor(x,dtype=torch.float).to(device)
                outputs=torch.zeros(y.shape).to(device)
                with torch.no_grad():
                    for i in range(input.size()[1]):
                        outputs[:,i,:]=ntm(input[:,i,:])
                ...
                i = 0
                '''print("task_name:",inp[0].split("/")[-1],"index:",index)
                print("outputs1s:",outputs[:1].shape)
                print("y1s:",y[:1].shape)
                print("correct_list:",correct_list)
                print("output_list:",outputs_list)
                print("nonzero:",np.unique(np.nonzero(outputs[:1]* m[:1])[1]))
