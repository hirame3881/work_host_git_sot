...
class DNC(nn.Module):
  ...
  def __init__(
    ...
    #inference
    ##self.infer_mod = nn.Linear(self.cell_size*2,self.cell_size*2)
    if self.rrnn_type:
      self.relation_mod =RelationalMemory(mem_slots=rel_config["mem_slots"],
                                          head_size=int(rel_config["mem_params"] / (rel_config["num_heads"] * rel_config["mem_slots"])),
                                          input_size=self.cell_size,
                                          dnc_nr_cells=self.nr_cells,
      ...
      ##self.dncmem_projector=nn.Linear(self.nr_cells,16)
      self.gat = GAT(nfeat=self.cell_size, 
                  nhid=gat_config["firstattention_hidden_size"], 
                  nclass=int(rel_config["mem_params"] / rel_config["mem_slots"]), 
                  dropout=gat_config["gat_dropout"], 
                  nheads=gat_config["gat_num_heads"],
                  alpha=gat_config["leakyrelu_alpha"])

    if self.gpu_id != -1:
      [x.cuda(self.gpu_id) for x in self.rnns]
      [x.cuda(self.gpu_id) for x in self.memories]
      #self.output.cuda()
      self.relation_mod.cuda()
  ...
  def _layer_forward(self, input, layer=0, hx=(None, None), pass_through_memory=True):
    (chx, mhx) = hx

    # pass through the controller layer
    input, chx = self.rnns[0](input.unsqueeze(1), chx)
    ...
    if pass_through_memory:
      read_vecs, mhx ,write_vector= self.memories[0](ξ, mhx)
    ...
    return output, (chx, mhx, read_vectors),write_vector
  ...
  def forward(self, input, hx=(None, None, None), reset_experience=False, pass_through_memory=True):
    ...
    rmem_hidden= self.relation_mod.initial_state(batch_size, trainable=True).cuda()
    ...
    routs =[None] * max_length
    read_vectors = None

    rmem_hidden = self.relation_mod.repackage_hidden(rmem_hidden) #~~??
    # pass through time
    for time in range(max_length):
      ...
      m = mem_hidden
      r_mem =rmem_hidden
      # pass through controller and memory
      outs[time], (chx, m, read_vectors),dnc_write_v = \
        self._layer_forward(inputs[time], 0, (chx, m), pass_through_memory)
      if self.rrnn_type:
        rrnn_input_after_gat= self.gat(m["memory"] , m["link_matrix"].squeeze(1))  
        #サイズ　memory (batch, m, w) link_mat (batch,1,m,m)
        ##r_out,r_mem =  self.relation_mod.forward_step(self.dncmem_projector(m["memory"].permute(0,2,1)).permute(0,2,1),r_mem,treat_input_as_matrix=True)
        ##r_out,r_mem =  self.relation_mod.forward_step(m["memory"],r_mem,dnc_write_v=dnc_write_v,treat_input_as_matrix=True)
        r_out,r_mem =  self.relation_mod.forward_step(rrnn_input_after_gat,r_mem,dnc_write_v=dnc_write_v,treat_input_as_matrix=True)
        
        rmem_hidden=r_mem
      ##r_out,r_mem =  self.relation_mod.forward_step(torch.mean(m["memory"], dim=1),r_mem,treat_input_as_matrix=False)
      ...
      ##infered_mem2slot =self.infer_mod(m["memory"][:,:2].view(batch_size,self.cell_size*2))
      ##outs[time] = T.cat([outs[time], infered_mem2slot], 1) 
      if self.rrnn_type:
        outs[time] = T.cat([outs[time], r_out],1)
      inputs[time] = outs[time]

    # pass through final output layer
    ##inputs = [self.output(i) for i in inputs]
    inputs =inputs
    outputs = T.stack(inputs, 1 if self.batch_first else 0) #timeごとで別れたtensorのリストを１つのTensorにするだけ
    #~ infer_module(m[Mr],query)
