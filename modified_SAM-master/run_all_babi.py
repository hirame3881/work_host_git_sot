...
from my_util import EarlyStopping
...
def train(config: Dict[str, Dict],
    ...
    for i in task_ids:
        ...
        print("task:",i,"train_raw_shape:",train_raw_data[0].shape)
    ...
    fileid_str_list=[str(id) for id in task_ids]
    fileid_str_list.append("mean")
    scorestoring =ScoreStoring(data_config,fileid_str_list=fileid_str_list,rowAttr="epoch",rowVal="Accuracy")
    earlystopping = EarlyStopping(patience=trainer_config["patience"], verbose=True)
    loss_list =[]
    ...
    for i in range(trainer_config["epochs"]):
        ...
        with torch.no_grad():
            ...
            for  k, va in valid_data_loaders.items():
                ...
                for story, story_length, query, answer in valid_data_loader:
                    ...
                    ##sentence_lens+=story.shape[1]
                    ##sentence_len_mother+=1
                print(f"validate acc task {k}: {task_acc/single_valid_samples}")
                ##if i<2:print(f"  mean_len:{sentence_lens/sentence_len_mother}")
                scorestoring.store(str(k),i,task_acc/single_valid_samples)
        ...
        scorestoring.store("mean",i,valid_acc)
        ...
        earlystopping((valid_loss), model) #callメソッド呼び出し
