...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size,dnc_nr_cells, num_heads=1, num_blocks=1, forget_bias=1., input_bias=0.,
        ...
        ##self.mem_slots_plus_input = self.mem_slots + 16
        self.mem_slots_plus_input = self.mem_slots + dnc_nr_cells
    ...
    def multihead_attention(self, memory):
        ...
        q_bs, k, v = torch.split(qkv_transpose, [self.key_size, self.key_size, self.value_size], -1)

        # scale q with d_k, the dimensionality of the key vectors
        q =q_bs * (self.key_size ** -0.5)
    ...
    def forward_step(self, inputs, memory, dnc_write_v,treat_input_as_matrix=False):
        """
        ...
        if self.gate_style == 'unit' or self.gate_style == 'memory':
            ...
            input_gate, forget_gate = self.create_gates(dnc_write_v, memory)
            ##input_gate, forget_gate = self.create_gates(torch.mean(inputs_reshape, dim=1).unsqueeze(dim=1), memory)
